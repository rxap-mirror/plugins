...
export class Builder {
  public static Run(options: PluginBuilderSchema, context: BuilderContext) {
  ...
  public async run(): Promise<BuilderOutput> {
    ...
    const guides = readFileSync(join(rootPath, 'GUIDES.md')).toString('utf-8');
  ...
  private getTemplate(basePath: string) {
    const readmeTemplateFile = readFileSync(
      join(basePath, 'README.md.handlebars')
    ).toString('utf-8');
  ...
  private getBuildersJson(basePath: string, packageJson: any) {
    const buildersJson: any = JSON.parse(
      readFileSync(join(basePath, 'builders.json')).toString('utf-8')
    );

    for (const builder of Object.values<any>(buildersJson.builders)) {
      builder.schema = JSON.parse(
        readFileSync(join(basePath, builder.schema)).toString('utf-8')
      );
